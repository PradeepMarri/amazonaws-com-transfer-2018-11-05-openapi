...
func ImportcertificateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=TransferService.ImportCertificate", cfg.BaseURL)
...
func CreateImportcertificateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=TransferService_ImportCertificate",
		mcp.WithDescription("Imports the signing and encryption certificates that you need to create local (AS2) profiles and partner profiles."),
		...
		mcp.WithString("PrivateKey", mcp.Description("")),
		mcp.WithString("Tags", mcp.Description("")),
		mcp.WithString("Usage", mcp.Required(), mcp.Description("")),
