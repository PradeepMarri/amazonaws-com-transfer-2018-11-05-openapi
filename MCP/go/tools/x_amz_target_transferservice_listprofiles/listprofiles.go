...
func ListprofilesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ListProfilesRequest
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=TransferService.ListProfiles%s", cfg.BaseURL, queryString)
		...
		var result models.ListProfilesResponse
...
func CreateListprofilesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=TransferService_ListProfiles",
		...
		mcp.WithString("ProfileType", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListprofilesHandler(cfg),
	}
}
